...
class ClientController extends Controller
{
    ...
    public function confirmDelivery(Request $request, $order_id)
    {
        ...
        return DB::transaction(function () use ($validated, $order_id) {
            ...
            if ($order->status !== 'shipping') {
                return response()->json(['message' => 'This order cannot be confirmed in its current status.'], 422);
            }
            ...
            foreach ($validated['items'] as $receivedItem) {
                $orderItem = $order->products->firstWhere('id', $receivedItem['order_item_id']);
                ...
                if ($receivedItem['received_quantity'] > $orderItem->quantity) {
                    return response()->json(['message' => 'Received quantity cannot exceed ordered quantity.'], 422);
                }
            ...
            return response()->json([
                'message' => $hasMissingStock ? 'Delivery confirmed with missing stock reported.' : 'Delivery confirmed successfully.',
                'order_id' => $order->id,
                'status' => $order->status,
